...
- CI required check として使う job 名は `static-analysis`、`test`、`native-build` とする（`static-analysis` 内で `check:react-versions`、`check:docs`、`type-check`、`lint`、`format:check`、`expo-doctor` を順次実行。`native-build` はネイティブ影響ファイル変更時のみビルド実行）。
...
- main branch protection / ruleset
  ...
  - Required checks: `static-analysis`、`test`、`native-build`。
...
## Operation Notes
- Dependabot PR は CI の `static-analysis`、`test`、`native-build` が通ってから手動で確認する。自動マージは行わない。
