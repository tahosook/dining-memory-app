...
review 画面には、local mock provider を使った最小の AI 入力補助が入りました。  
Settings には local AI spike の事前許可が入りましたが、current build の on-device runtime はまだ未組み込みです。  
...
- AI 入力補助
  ...
  - local runtime prototype は未組み込みで、current build では disabled reason を返す
- 統計
  - 現在は軽量なサマリーのみ
- 検索
