...
Settings には local AI spike の事前許可と local AI runtime status が入り、条件を満たさない build では引き続き disabled reason を返します。  
...
- review 画面で、明示操作に限って AI 候補を取得し、料理名、料理ジャンル、自炊 / 外食候補を入力へ反映する
- 記録一覧を日付ごとに見返し、詳細画面で大きい写真を確認する
- 記録を詳細画面から編集・削除する
- 記録詳細から、共有シート経由で気に入った写真だけ X 投稿に進む
- テキスト検索、料理ジャンル、場所フィルター、自炊フィルターで探し、結果からそのまま編集・削除する
- local runtime と vectors がそろう build では、text/filter search に semantic hit を additive に混ぜて探せる
- 総記録数、自炊比率、よく記録する料理ジャンル・場所を確認し、更新失敗時は再試行する
- 設定画面でプライバシー方針、local AI runtime status、ローカルデータ削除を確認する
...
- AI 入力補助
  ...
  - Settings で semantic search と meal input assist の ready / unavailable reason、固定 model path を確認できる
- 統計
  - 現在は軽量なサマリーのみ
- 検索
  - text、料理ジャンル、場所、自炊フィルターを基準にしたまま、semantic hit を additive に混ぜる最小 hybrid path まで
  - local runtime unavailable、model missing、embedding missing のときは current text/filter search に戻る
- 画像表示
...
- 場所フィルター
- 自炊のみフィルター
- local semantic hit の additive merge と fallback
- 検索結果からの詳細画面遷移
...
- ローカル AI による料理認識
- クラウド API を使った補完解析
- 検索 quality 改善のための rerank / relevance tuning
- バックアップ、エクスポート、復元
- より深い統計・傾向分析
