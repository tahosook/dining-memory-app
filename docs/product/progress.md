...
## Summary
Dining Memory は、撮影して保存し、一覧・検索・期間別の統計サマリーで振り返るところまでが動く段階にあります。
...
- review 画面と詳細編集モーダルで、明示操作に限って AI メモ下書きを作成し、ユーザーが押したときだけメモ欄へ追加する
- 記録一覧を日付ごとに見返し、詳細画面で大きい写真を確認する
- 記録を詳細画面から編集・削除し、自炊記録では自炊スタイルも扱う
...
- メイン写真保存時のネイティブリサイズ・JPEG圧縮（1600px / JPEG 80%）およびフォールバック機構（Issue #86）
- 写真世代（photo generation）ベースの非同期サムネイル処理による写真回転・サムネイルライフサイクルの分離と競合耐性確保（Issue #87）
- 記録一覧（RecordsScreen）におけるページネーション（50件単位の無限スクロール・追加読み込み）機構（Issue #89）

### 一部実装
- AI 入力補助
  - 現在は review 画面と詳細編集モーダルの user-triggered なメモ下書き作成のみ
  - 作成された `noteDraft` は明示的な `メモに追加` 操作で notes にだけ追記する
  ...
  - MediaPipe custom food classifier 向け static-image classification path は Android native bridge まで実装済みだが、default runtime と Settings/readiness wiring は未接続
...
- 写真撮影
- 撮影後レビュー
- review / 詳細編集上の `AIでメモを作成` ボタン、`noteDraft` card、`メモに追加` 操作
- 保存前にネイティブリサイズ（最大長辺1600px / JPEG 80% / アスペクト比維持）と安全なフォールバックで端末内向けに軽量化
- 手動入力
- 保存後に Records へ即時遷移
...
- 自炊 / 外食件数
- 自炊比率
- 7日 / 今月 / 先月 / 全期間の period tabs
- 短いふりかえり文
- 自炊・外食 balance bar
- よく記録する料理ジャンル・場所の Top 3 ranking
- loading / error 表示
- エラー時の再試行導線
