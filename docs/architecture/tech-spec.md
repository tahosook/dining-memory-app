...
- For list display performance, an in-app display thumbnail (max 320x320, JPEG 70%) is strictly separated from the main photo (1600px, JPEG 80%). It is generated asynchronously after Meal creation or lazily filled during list display, saved to an app-local stable path (`${photo_path}-thumb.jpg`), and recorded via `photo_thumbnail_path`. Generation runs through a shared queue with concurrency limit (max 2), in-flight deduplication, and stale-update prevention. When a photo is rotated, the main photo is rotated and saved, `photo_thumbnail_path` is reset to null, and a 320px thumbnail is re-requested via the shared queue, ensuring high-resolution main photos are never directly reused as thumbnails. Thumbnails do not require EXIF or MediaLibrary registration, and failures preserve the original photo and meal creation without blocking save or list rendering.
